---
title: "Supported Chains: Ethereum, Base, BNB, and 60+ Networks"
sidebarTitle: "Supported Chains"
description: "View all mainnet and testnet chains supported by Etherscan API V2, including free and paid tier availability across the full EVM ecosystem."
---

A single Etherscan API key covers all chains listed below. To query a specific chain, include the `chainid` parameter in every API V2 request — for example, `chainid=1` for Ethereum Mainnet or `chainid=137` for Polygon.

<Tip>
  Source code and ABI endpoints are available on **all chains** for every API plan, including the Free tier.
</Tip>

For Solana (SOL) data, visit the [Solscan APIs](https://solscan.io/apis), available separately.
